    ...
    public function up(): void
    {
        $procedure = <<<SQL
                ...
                BEGIN
                    ...
                    IF p_supplier_bussiness_name IS NULL OR p_supplier_bussiness_name = '' THEN
                        RAISE EXCEPTION 'Supplier business name cannot be null or empty';
                    END IF;

                    IF p_supplier_primary_email IS NOT NULL THEN
                        IF NOT p_supplier_primary_email ~* '^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}$' THEN
                            RAISE EXCEPTION 'Invalid primary email format';
                        END IF;
                    END IF;

                    IF p_supplier_secondary_email IS NOT NULL THEN
                        IF NOT p_supplier_secondary_email ~* '^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}$' THEN
                            RAISE EXCEPTION 'Invalid secondary email format';
                        END IF;
                    END IF;
